...
<div x-data="{ activeTab: 'home' }" class="min-h-screen">
    ...
    <header class="sticky top-0 z-50 bg-[#F5F4EE]/95 backdrop-blur-md border-b border-[#E0DED7]/40">
        ...
        <nav class="hidden lg:flex items-center gap-10">
            ...
            <a
                href="{{ route('pos') }}"
                class="text-[11px] uppercase tracking-[0.2em] font-black {{ Route::is('pos') ? 'text-[#8C1717]' : 'text-[#3D3833]/70' }} hover:text-[#8C1717] transition"
            >
                Shop
            </a>

            <a
                href="{{ route('customer.home') }}#about-section"
            ...
            <a
                ...
                class="text-[11px] uppercase tracking-[0.2em] font-black {{ Route::is('order_history') ? 'text-[#8C1717]' : 'text-[#3D3833]/70' }} hover:text-[#8C1717] transition"
            >
                Transaction History
            </a>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="relative" x-data="{ open: false }">
                    ...
                    <button @click="open=!open" class="w-11 h-11 rounded-full overflow-hidden border border-[#3D3833]/10 hover:scale-105 transition">
                        @auth
                            <img src="{{ auth()->user()->profile_photo ?? 'https://i.pravatar.cc/100' }}" class="w-full h-full object-cover">
                        @else
                    ...
                    <div x-show="open" @click.outside="open=false" x-transition class="absolute right-0 mt-4 w-[260px] bg-[#F8F5F2] border border-[#E5E0DA] rounded-[28px] shadow-[0_25px_50px_rgba(0,0,0,0.08)] overflow-hidden z-50">
                        @auth
                            <div class="p-5">
                                <h3 class="font-black text-[#2C2623]">{{ auth()->user()->name }}</h3>
                                <p class="text-sm text-stone-400 mt-1">{{ auth()->user()->email }}</p>
                            </div>
                            <div class="border-t border-[#E5E0DA]"></div>
                            <div class="p-3 space-y-1">
                                <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl hover:bg-white transition text-sm font-black uppercase tracking-wide">✨ Edit Profile</a>
                        ...
                        @else
                            <div class="p-5">
                                ...
                                <div class="space-y-3 mt-6">
                                    <a href="{{ route('login') }}" class="block text-center bg-[#8C1717] text-white py-3 rounded-2xl font-black uppercase tracking-wider text-sm">Login</a>
                                    <a href="{{ route('register') }}" class="block text-center bg-[#F1ECE8] text-[#2C2623] py-3 rounded-2xl font-black uppercase tracking-wider text-sm">Register</a>
                                </div>
                            </div>
                        @endauth
                    </div>
